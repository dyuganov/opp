...
void MpiV2NonlinearConjugateGradient(double* A, double* b, double* x, int rank, int size){
    ...
    if(rank == 0) b_vecLength = getVectorLength(b);
    ...
    for(int i = 0; i < vectorPartSize; ++i) Ax_vecPart[i] = 0;
    mulMatrixAndVectorParts(size, A_matrixPart, x_vecPart, Ax_vecPart);
    ...
    while(epsilonCheck >= EPSILON){
        //Az^n
        for(int i = 0; i < vectorPartSize; ++i) Az_vecPart[i] = 0;
        mulMatrixAndVectorParts(size, A_matrixPart, z_vecPart, Az_vecPart);
